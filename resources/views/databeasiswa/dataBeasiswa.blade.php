...
<body>
    ...
    <table class="border-collapse border border-slate-400 mx-auto mt-10 mb-5">
        <tr class="bg-green-400">
            ...
            @if(Auth::user())
            @if(Auth::user()->role == "Admin")
            <th colspan="2" class="px-10 border border-slate-400">Aksi</th>
            @endif
            @endif
        </tr>
        @php
        ...
        <tr class="border text-center ">
            ...
            <td class="border border-slate-400"> <!-- Tambah baris ini -->
                <a href="{{route('edit-data-beasiswa', [$item->id, $year])}}" class="bg-green-600 px-4 py-1 rounded-md text-white font-bold uppercase" data-toggle="tooltip" title='Edit'>Edit</a>
            </td>
            @endif
            @endif
